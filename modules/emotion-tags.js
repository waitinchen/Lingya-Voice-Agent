 ...
 * @param {Array} context.conversationHistory - 對話歷史（可選）
 * @returns {Array<string>} 推斷的語氣標籤列表（0-3個）
 */
export function inferEmotionTags(text, context = {}) {
  ...
  const { userIdentity, conversationHistory = [] } = context;
...
/**
 * 從對話歷史中提取情緒趨勢（用於背景情緒權重融合）
 * @param {Array} conversationHistory - 對話歷史
 * @returns {Object} 情緒趨勢 { dominant: string, tags: Array<string> }
 */
export function extractEmotionTrend(conversationHistory = []) {
  if (!conversationHistory || conversationHistory.length === 0) {
    return { dominant: 'neutral', tags: ['neutral'] };
  }

  // 分析最近幾輪對話的情緒
  const recentMessages = conversationHistory.slice(-5); // 最近5輪
  const emotionCounts = {};

  for (const msg of recentMessages) {
    if (msg.role === 'user') {
      const inferredTags = inferEmotionTags(msg.content || '');
  ...
  const tags = sorted.slice(0, 2).map(([tag]) => tag);

  return { dominant, tags: tags.length > 0 ? tags : ['neutral'] };
}
